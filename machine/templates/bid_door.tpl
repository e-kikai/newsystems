...
<div class="bid_search_comment">さあ、お気に入りの中古機械を探しだしてみてください！！
  {if !empty($bidOpen.list_pdf)}
    <a class="list_pdf" href="{$_conf.media_dir}pdf/{$bidOpen.list_pdf}" target=="_blank"><img src="./imgs/pdficon_large.png">商品リスト印刷用PDF</a>
  {/if}
</div>
...
<h2>地域・都道府県からさがす</h2>
  {foreach $regionList as $r}
    <div class="region xl_{$r.region}" data-xl="{$r.region}">{$r.region} ({$r.count})</div>
    <div class="xl_large_genre_area xl_{$r.region}">
      <a href="bid_list.php?o={$bidOpenId}&r={$r.region}" class="genre all_genre">
        <span class="check_label">{$r.region}</span><span class="count">({$r.count})</span>
      </a>
      {foreach $stateList as $s}
        {if $s.region_id == $r.id}
           <a href="bid_list.php?o={$bidOpenId}&s={$s.state}" class="genre xl_{$r.region}">
             <span class="check_label">{$s.state}</span><span class="count">({$s.count})</span>
           </a>
        {/if}
      {/foreach}
    </div>
...
<div class="xl_area">
<h2>すべての商品リスト</h2>
  <a class="all_list" href="bid_list.php?o={$bidOpenId}">すべての商品リスト ({$count})</a>
</div>
*}
...
<div class="xl_area">
  ...
  <div id="region_tab">
    ...
    <div id="tab_all" class="tab_area">
      ...
      {*
      <a class="xl_genre" href="bid_list.php?o={$bidOpenId}&yama=1">
        新春初売り・一山商品
        <img class="lazy" src='imgs/blank.png' data-original="{$_conf.media_dir}machine/thumb_f_20141210132218_0.jpg" alt='' />
      </a>
      *}
