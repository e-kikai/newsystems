...
{block 'main'}
  ...
  <div class="formula_area">
    <p>
      ...
      うち、当月の新規登録数は {$res.machinesCreate|number_format}件 (一日平均約
      {($res.machinesCreate / $dateCount)|number_format:0|floatval}件)、
  ...
  *}
    ...
    <p>
      削除(売却)数は {$res.machinesDelete|number_format}件 (一日平均約
      {($res.machinesDelete / $dateCount)|number_format:0|floatval}件)、
    ...
    <p>
      ...
      問い合わせを行ったユーザ数は {$res.contactsUser|number_format}人 (一人平均約
      {($res.contactsAll / $res.contactsUser)|number_format:0|floatval}件)でした。
    </p>

    <p>
      うち、在庫情報についての問合せは {$res.contactsMachine|number_format}件 (一日平均約
      {($res.contactsMachine / $dateCount)|number_format:0|floatval}件)、<br />
    ...
    <p>
      登録会社への問合せは {$res.contactsCompany|number_format}件 (一日平均約
      {($res.contactsCompany / $dateCount)|number_format:0|floatval}件)、<br />
      事務局への問合せは {$res.contactsSystem|number_format}件 (一日平均約
      {($res.contactsSystem / $dateCount)|number_format:0|floatval}件)でした。
    </p>
    ...
    <p>
      機械詳細ページアクセス数について、<br />
      総アクセス数は {$res.ActionlogMachine|number_format}アクセス (一日平均約
      {($res.ActionlogMachine / $dateCount)|number_format:0}件)、<br />
      アクセスしたユーザ数は {$res.ActionlogMachineUniq|number_format}人 (一人平均約
      {($res.ActionlogMachine / $res.ActionlogMachineUniq)|number_format:0|floatval}件)、<br />
    ...
    <p>
      ...
      利用ユーザ数は {$res.ActionlogPDFUniq|number_format}人 (一人平均約
      {($res.ActionlogPDF / $res.ActionlogPDFUniq)|number_format:0|floatval}件)でした。
    </p>
    ...
    {foreach $res.bids as $bi}
      ...
      <ol class="lanking">
        {foreach $bi.logBMRanking as $c}
          <li>{$c.name} {$c.maker} {$c.model} {$c.year}(最低入札価格:{($c.min_price/1000)|number_format}千円)の
            {$c.count|number_format}件</li>
        {/foreach}
      </ol>
