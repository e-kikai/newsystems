        ...
        'alternatives': [
            {
                'alternative': function($this) {
                    $this.addClass('contact-long').html('お問い合わせ・入札依頼はこちら');
                },
                'value': 'お問い合わせ・入札依頼はこちら',
            },
            {'alternative': '入札依頼', 'value': '入札依頼'},
        ]
    });
    */
    ...
    <table class="spec">
      ...
      <tr class="">
        ...
        <td class='location'>
          ...
          {if $machine.addr3}
            <a class="button accessmap" href="https://maps.google.co.jp/maps?f=q&amp;q={$machine.addr1|escape:"url"}{$machine.addr2|escape:"url"}{$machine.addr3|escape:"url"}+({$machine.location|escape:"url"})&source=embed&amp;hl=ja&amp;geocode=&amp;ie=UTF8&amp;t=m&z=14" target="_blank">MAP</a>
          {/if}
        </td>
      </tr>
      <tr class="label_area">
          ...
          {if !empty($machine.pdfs)}
            {foreach $machine.pdfs as $key => $val}
              <a href="{$_conf.media_dir}machine/{$val}"
                class="label pdf" target="_blank" title="クリックで資料PDFを閲覧できます">PDF:{$key}</a>
            {/foreach}
          {/if}
        </td>
      </tr>
      ...
      <table class="form spec">
        <tr class="bid">
          ...
          <td>
            ...
            <a href="/admin/bid_fee_sim.php?o={$bidOpenId}" onclick="window.open('/admin/bid_fee_sim.php?o={$bidOpenId}','','scrollbars=yes,width=850,height=450,');return false;">
              支払・請求額シミュレータ
            </a>
          </td>
        </tr>
      ...
      *}
      {if !empty($smarty.session.bid_batch) && !empty($smarty.session.bid_batch[$machine.id])}
        <button class="mylist" value="{$machine.id}" style="display:none;"
          onclick="ga('send', 'event', 'bid_batch', 'set', '{$machine.id}', 1, true);"
          ><span class="mylist_pluse">＋</span>一括問い合わせに追加</button>
        <button class="delete_mylist" value="{$machine.id}"
          onclick="ga('send', 'event', 'bid_batch', 'delete', '{$machine.id}', 1, true);"
          >一括問い合わせ削除</button>
      {else}
        <button class="mylist" value="{$machine.id}"
          onclick="ga('send', 'event', 'bid_batch', 'set', '{$machine.id}', 1, true);"
...
<div class="contactbar">
  <div class="contactbar_area">
    ...
    <a class="contact" href="contact.php?o={$bidOpenId}&bm={$machine.id}" target="_blank"
      onclick="ga('send', 'event', 'bid_contact', 'detail_contactbar');"
      >フォームから問い合わせ</a>
    {*
    <div class="tel_label">お問い合わせTEL</div>
    <div class="contact_tel">{$company.contact_tel|escape|replace:',':",<br />" nofilter}</div>
    *}
    <a class="button contact_tel subwindow" href="bid_detail_tel.php?m={$machine.id}"
      onclick="ga('send', 'event', 'bid_contact', 'detail_tel_contactbar');"
      >電話で問い合わせ</a>
  </div>
</div>
{else}
