...
class Pdf
{
    ...
    public function makeSagefuda($title, $bidMachineList)
    {
        foreach ($bidMachineList as $key => $m) {
            ...
            // 罫線
            if (!empty($barY)) { $this->_p->Line(0, $barY, self::A4_HEIGHT, $barY); }
            ...
            // メーカー、型式、年式
            $mes = $m['maker'];
            if (!empty($m['model'])) { $mes .= ' ' . $m['model']; }
            if (!empty($m['year']))  { $mes .= ' ' . $m['year'] . '年式'; }
    ...
    public function makeReceipt($bidOpen, $sumList, $isSeri = false)
    {
        ...
        foreach($sumList as $s) {
            // 請求金額がある場合のみ、請求証を作成
            if ($s["final_payment"] > $s["final_billing"]) { continue; }
    ...
    public function makeCompanySeikyu($companyList, $date)
    {
        ...
        foreach ($companyList as $c) {
            ...
            $price = 0;
            $title = '';
            if ($c['rank'] >=      202) { $price = '80000'; $title = '特別会員 年会費'; }
            // else if ($c['rank'] >= 201) { $price = '5000';  $title = '支店・営業所アカウント発行手数料'; }  // 2015のみ
            else if ($c['rank'] >= 201) { $price = '0'; }  // 2016以降
            else if ($c['rank'] >= 200) { $price = '30000'; $title = 'A会員 年会費'; }
            else if ($c['rank'] >= 100) { $price = '12000'; $title = 'B会員 年会費'; }
