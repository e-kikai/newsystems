...
<meta name="keywords"
  content="{$machine.name},{$machine.maker},{$machine.model},
          ...
          {if $machine.addr3}
            ...
            *}
            {if preg_match('/^[a-zA-Z0-9]/',$company.addr1)}
              <a class="accessmap" href="https://maps.google.co.jp/maps?f=q&amp;q={$company.lat|escape:"url"} {$company.lng|escape:"url"}+({$company.company|escape:"url"})&source=embed&amp;hl=ja&amp;geocode=&amp;ie=UTF8&amp;t=m" style="color:#0000FF;text-align:left" target="_blank">MAP</a>
            {else}
              <a class="accessmap" href="https://maps.google.co.jp/maps?f=q&amp;q={$machine.addr1|escape:"url"}{$machine.addr2|escape:"url"}{$machine.addr3|escape:"url"}+({$machine.location|escape:"url"})&source=embed&amp;hl=ja&amp;geocode=&amp;ie=UTF8&amp;t=m&z=14" target="_blank">MAP</a>
            {/if}
          {/if}
        </td>
      </tr>
      ...
      <tr class="label_area">
        <td colspan="2">
          {if !empty($bidMachineIds[$machine.id])}
            <a href="bid_detail.php?m={$bidMachineIds[$machine.id].bid_machine_id}"
              class="label bid" target="_blank" title="クリックで出品商品ページへリンクします">{$bidMachineIds[$machine.id].bid_title} 出品中
              {*
