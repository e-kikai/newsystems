...
<dl class="total">
  ...
  {if $sum.final_billing > $sum.final_payment}
    <dt class="result">差引落札請求額</dt><dd class="result">{($sum.final_billing - $sum.final_payment)|number_format}円</dd>
  {else}
    <dt class="result">差引出品支払額</dt><dd class="result">{($sum.final_payment - $sum.final_billing)|number_format}円</dd>
  {/if}
</dl>
