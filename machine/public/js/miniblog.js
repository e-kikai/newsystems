...
$(function() {
    ...
    */
    ...
    $('<div class="miniblog_area">')
      .html('メッセージを書きこんで「送信する」をクリックしてください<br />書きこんだ内容は会員ページに表示されます')
      .appendTo('body');
    $('<textarea class="miniblog_input"></textarea>').appendTo('.miniblog_area');
    $('<button type="button" class="miniblog_submit">メッセージを送信する</button>').appendTo('.miniblog_area');

    //// @ba-ta 20150114 親ID、レスポンスIDの追加 ////
    $('<input type="hidden" class="parent_id" value="" />').appendTo('.miniblog_area');
    $('<input type="hidden" class="response_id" value="" />').appendTo('.miniblog_area');
    ...
    $('.miniblog_open, .miniblog_response').click(function() {
        ...
        if (thisId) {
            var thisText = $('.info_contents[data-id=' + thisId + ']').text()
                .replace(/(^|\n\r|\r|\n)/g, "$1> ") + "\n\n";
    ...
    $('button.miniblog_submit').click(function() {
        ...
        function(data) {
            ...
            $('.miniblog_input').val('');
            if (data == 'success') { cjf.showAlert('メッセージを送信しました'); }
            else                   { cjf.showAlert(data); }
        });
    });
});
