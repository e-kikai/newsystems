...
{block 'main'}
  ...
  {if !empty($machine)}
    ...
    *}
          <img class='noimage' src='{$_conf.site_uri}imgs/noimage.png' alt="中古{$m.name} {$m.maker} {$m.model}"
            title="{$m.name} {$m.maker} {$m.model}" />
        {else}
          <div class='top_image'>
            <div id='viewport'>
              {if !empty($machine.top_img)}
                <a class="zoom" href="{$_conf.media_dir}machine/{$machine.top_img}" rel='gal1'>
                  <img class="zoom_img" src="{$_conf.media_dir}machine/{$machine.top_img}"
                    alt="中古{$machine.name} {$machine.maker} {$machine.model}" />
                </a>
              {/if}
            </div>
          </div>
          ...
          <div class='images'>
            {if !empty($machine.imgs)}
              {if !empty($machine.top_img)}
                <a class="img" href='javascript:void(0);' rel="{literal}{{/literal}
                gallery:'gal1',
        ...
        {if !empty($machine.youtube) && preg_match_all('/([\w\-]{11})/', $machine.youtube, $res)}
          <div class="youtube">
            {foreach $res[0] as $y}
              <iframe width="400" height="300" src="https://www.youtube.com/embed/{$y}?rel=0" frameborder="0"
      ...
      *}
        ...
        <div class='contact_area'>
          {if !empty($machine.contact_mail)}
            <a class='contact_link' href='contact.php?m={$machine.id}'><img
                src='{$_conf.site_uri}imgs/contact_button.png' /></a>
          {/if}
        ...
        {if $machine.addr3}
          ...
          <a href="https://maps.google.co.jp/maps?f=q&amp;q={$machine.addr1} {$machine.addr2} {$machine.addr3}+({$machine.location})&source=embed&amp;hl=ja&amp;geocode=&amp;ie=UTF8&amp;t=m"
