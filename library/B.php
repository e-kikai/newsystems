...
class B
{
     ...
     */
    protected static $_on50Array = array(
        'ア行'   => '/^[アイウエオァィゥェォヴ]/u',
        'カ行'   => '/^[カキクケコガギグゲゴ]/u',
        'サ行'   => '/^[サシスセソザジズゼゾ]/u',
        'タ行'   => '/^[タチツテトダヂヅデドッ]/u',
        'ナ行'   => '/^[ナニヌネノ]/u',
        'ハ行'   => '/^[ハヒフヘホバビブベボパピプペポ]/u',
        'マ行'   => '/^[マミムメモ]/u',
        'ヤ行'   => '/^[ヤユヨャュョ]/u',
        'ラ行'   => '/^[ラリルレロ]/u',
        'ワ行'   => '/^[ワヰヱヲヮ]/u',
     ...
     * @access protected static
     */
    protected static $_dependentChars = array(
        // /* --- 0x2100 - 0x2138 (文字種記号) --- */
        // // 0x2100 - 0x210F
        // "\xE2\x84\x80" => "a/c",
        // "\xE2\x84\x81" => "a/s",
     ...
     * @return string fメソッドの戻り値(フィルタリング後の文字列)
     */
    // fメソッドのエイリアス
     ...
     * @return mixed  行番号
     */
    public static function get50onRow($str)
    {
        // 対象文字列の1文字目取得→カタカナ
        $firstStr = mb_substr($str, 0, 1);
        $firstStr = mb_convert_kana($firstStr, "KVrnC");

        // 50音配列を検索
        foreach (self::$_on50Array as $key => $re) {
            if (preg_match($re, $firstStr)) {
                return $key;
            }
        }
        return count(self::$_on50Array);
    }

    /**
     * 50音一覧初期化
     *
     * @return array 50音一覧（中身は空）
     */
    public static function get50onListInit()
    {
        return array(
            'ア行'   => array(),
            'カ行'   => array(),
    ...
    /**
     * DB保存用に2次元配列のJSONをエンコード
     *
     * @access public static
     * @param  array $data        JSONエンコードするデータ
    ...
    /**
     * DBテーブルから取得した2次元配列のJSONをデコード
     *
     * @access public static
     * @param  array $table       JSONデコードするテーブル
     * @param  mixed $jsonColumns JSONデコード対象カラム
     * @return array デコードしたテーブル
     */
    ...
    /**
     * DBテーブルから取得したRowのJSONをデコード
     *
     * @access public static
     * @param  array $table       JSONデコードするrow
     * @param  mixed $jsonColumns JSONデコード対象カラム
     * @return array デコードしたrow
     */
    public static function decodeRowJson($row, $jsonColumns)
    {
        ...
        foreach ((array)$jsonColumns as $col) {
            ...
            $row[$col] = empty($row[$col]) ? array() : json_decode($row[$col], true);
        }
     ...
     * @access public static
     * @param  mixed $arr
     * @return array フィルタリングした配列
     */
    public static function arrayIntFilter($arr)
    {
        $temp = array();
        foreach ((array)$arr as $val) {
            $ival = intval($val);
            if ($ival != 0 && !in_array($ival, $temp)) {
                $temp[] = $ival;
            }
        }
        return $temp;
     ...
     * @access public static
     * @param  mixed $arr
     * @return array フィルタリングした文字列
     */
    public static function replaceDependent($str)
    {
        return str_replace(array_keys(self::$_dependentChars), array_values(self::$_dependentChars), $str);
    }

    /**
     * sjisのファイル内容をutf-8にデコードして開き直す
     *
     * @access public static
     * @param  mixed $path ファイルパス
     * @return utf-8に変換したファイルハンドル
     */
    public static function file2utf($path)
    {
    ...
    public static function u2s($string)
    {
        return mb_convert_encoding($string, 'sjis-win', 'utf-8');
    }

    /**
     * 配列をルールに沿ってCSV化、sjisに変換して取得する
     *
     * @access public static
     * @param array $header CSVヘッダ
     * @param array $data CSV化する配列
     * @return string SJIS変換したCSVデータ
     */
    public static function makeCsv($header, $data)
    {
        // ファイルとして出力ストリームを開く・出力ストリームを捕捉する
        ob_start();
     ...
     * @return string 曜日(漢字表記)文字列
     */
    public static function strtoweek($str)
    {
