<tbody class="machine">
  ...
  <tr class='machine_spec'>
    <td class='spec' colspan='6'>
      ...
      {{for _render_pdfs}}
        <a href="{{>media_dir}}machine/{{>data}}" class="label pdf" target="_blank">PDF:{{>key}}</a>
      {{/for}}

      {{if catalog_id}}
        <a href="http://catalog.zenkiren.net/catalog_pdf.php?id={{>catalog_id}}" class="label catalog"
          target="_blank">電子カタログ(会員のみ公開)</a>
      {{/if}}
