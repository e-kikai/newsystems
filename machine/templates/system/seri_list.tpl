...
<div class="bid_open">
  <dl class="bid_date">
    <dt>出品期間</dt>
    <dd>{$bidOpen.entry_start_date|date_format:'%Y/%m/%d %H:%M'} ～ {$bidOpen.bid_end_date|date_format:'%m/%d %H:%M'}</dd><br />
    <dt>開催期間</dt>
    <dd>{$bidOpen.seri_start_date|date_format:'%Y/%m/%d %H:%M'} ～ {$bidOpen.seri_end_date|date_format:'%m/%d %H:%M'}</dd><br />
  </dl>
</div>
