...
<div class="companybid">
  ...
  <div class="companybid-comment">
    {$companybidOpen.comment|escape|auto_link|nl2br|regex_replace:"/(\(\((.*)\)\))/":'<span class="companybid-strong">$2</span>'|regex_replace:"/(\{\{(.*)\}\})/":'$2 <a class="accessmap" href="https://maps.google.co.jp/maps?f=q&amp;q=$2&source=embed&amp;hl=ja&amp;geocode=&amp;ie=UTF8&amp;t=m&z=14" target="_blank">MAPはこちら</a>' nofilter}
  </div>
</div>
        ...
        <div class="xl_large_genre_area xl_{$x.xl_genre_id}">
          ...
          {foreach $largeGenreList as $l}
            {if $l.xl_genre_id == $x.xl_genre_id}
               <a href="companybid_list.php?o={$companybidOpenId}&l={$l.large_genre_id}" class="genre xl_{$l.xl_genre_id}">
      ...
      <div id="tab_{$lo@key}" class="tab_area">
        {foreach $lo.xl_genre_list as $x}
          ...
          <div class="xl_large_genre_area xl_{$lo@key}_{$x.xl_genre_id}">
            <a href="companybid_list.php?o={$companybidOpenId}&lo={$lo.location}&x={$x.xl_genre_id}" class="genre all_genre">
              <span class="check_label">すべての{$x.xl_genre}</span><span class="count">({$x.count})</span>
            </a>
            {foreach $lo.large_genre_list as $l}
              {if $l.xl_genre_id == $x.xl_genre_id}
                 <a href="companybid_list.php?o={$companybidOpenId}&lo={$lo.location}&l={$l.large_genre_id}" class="genre xl_{$l.xl_genre_id}">
                   <span class="check_label">{$l.large_genre}</span><span class="count">({$l.count})</span>
                 </a>
              {/if}
            {/foreach}
